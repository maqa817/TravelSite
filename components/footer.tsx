...
import { Instagram, Facebook, MessageCircle, Phone, Mail, MapPin, ArrowRight } from "lucide-react"
...
export default function Footer() {
  ...
  const socialLinks = [
    ...
    {
      name: "Snapchat",
      href: "https://www.snapchat.com/add/travelbakufromarabia",
      icon: ({ className }: { className?: string }) => (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className={className}>
  ...
  return (
    <footer className="bg-emerald-900 dark:bg-emerald-950 text-white" dir={dir}>
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={staggerChildren} className="lg:col-span-2">
            <motion.h3 className="text-2xl font-bold mb-6" variants={fadeInUp}>
              TravelBakufromArabia
            </motion.h3>
            <motion.p className="text-emerald-100 mb-8 text-lg" variants={fadeInUp}>
              {t("footer.aboutShort")}
            </motion.p>
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={staggerChildren}>
            <motion.h3 className="text-xl font-bold mb-6" variants={fadeInUp}>
              {t("footer.quickLinks")}
            </motion.h3>
            <motion.ul className="space-y-5" variants={staggerChildren}>
              {[
                ...
                <motion.li key={index} variants={fadeInUp}>
                  <Link
                    href={item.href}
                    className="text-emerald-100 hover:text-white transition-colors duration-300 flex items-center group text-lg"
                  >
                    <ArrowRight className="h-5 w-5 mr-3 transform group-hover:translate-x-1 transition-transform duration-300" />
                    {item.name}
                  </Link>
                </motion.li>
              ))}
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={staggerChildren}>
            ...
            <motion.ul className="space-y-5" variants={staggerChildren}>
              <motion.li className="flex items-start" variants={fadeInUp}>
                <Mail className="h-6 w-6 mr-3 mt-0.5 text-emerald-300" />
                <a
              ...
              <motion.li className="flex items-start" variants={fadeInUp}>
                <MapPin className="h-6 w-6 mr-3 mt-0.5 text-emerald-300" />
        ...
        <motion.div
          className="mt-12 pt-8 border-t border-emerald-800 text-center text-emerald-200"
        ...
        >
          <p>
            &copy; {new Date().getFullYear()} TravelBakufromArabia. {t("footer.rights")}
          </p>
        </motion.div>
      </div>
    </footer>
